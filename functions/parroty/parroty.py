...
class Parroty():
    def __init__(self):
        self.tokenizer = tokenizer = GPT2Tokenizer.from_pretrained('gpt2-large', bos_token='<|startoftext|>',
                                                                   eos_token='<|endoftext|>', pad_token='<|pad|>')
        self.configuration = GPT2Config.from_pretrained('gpt2-large', output_hidden_states=False)
        self.model = GPT2LMHeadModel.from_pretrained("gpt2-large", config=self.configuration)
        self.model.resize_token_embeddings(len(self.tokenizer))
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model.to(self.device)

    def create_model(self, data, epochs=1, batch_size=4):

        # We expect data to be passed as a list of strings representing sentences,
        # we can add in a capability to read a .txt file
    ...
    def generate_text_from_model(self, context):
        self.model.eval()
    ...
    class GPT2Dataset(Dataset):
        def __init__(self, txt_list, tokenizer, gpt2_type="gpt2-large", max_length=500):
            self.input_ids = []
            self.attn_masks = []
...
# Adapted from Blather by __________@github.com